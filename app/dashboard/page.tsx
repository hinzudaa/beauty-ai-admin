  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {isLoading
          ? [...Array(4)].map((_, i) => <div key={i} className={`${CARD} animate-pulse h-[100px]`} />)
          : stats.map((s) => (
                  ...
                  return (
                    <div key={feature}>
                      <div className="flex justify-between text-sm mb-1.5">
                        ...
                        <span className="text-white font-medium">{count.toLocaleString()} <span className="text-white/30 text-xs">({pct}%)</span></span>
                      </div>
        ...
        <div className={CARD}>
          ...
          <div className="space-y-2">
            {[
              ...
              { href: "/dashboard/settings", label: "Үнэ тохиргоо →"             },
            ].map((l) => (
          ...
          <div className="mt-5 pt-4 border-t border-white/[0.06]">
            <p className="text-xs text-white/30 mb-3">Хурдан статистик</p>
            {[
              { label: "Баталгаажаагүй нэхэмжлэх", value: (data?.totalPayments ?? 0) - (data?.paidPayments ?? 0) },
              { label: "Дундаж орлого (нэг худалдан авалт)", value: data?.paidPayments ? `${Math.round((data.totalRevenue) / data.paidPayments).toLocaleString()}₮` : "—" },
            ].map((r) => (
